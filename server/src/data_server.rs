...
fn mk_query(event_type: &str) -> String {
    format!("WITH running_totals AS (
        SELECT
            toStartOfInterval(decision_timestamp, INTERVAL 1 HOUR) AS interval_start,
            countIf(action_type = '{}') OVER (ORDER BY toStartOfInterval(decision_timestamp, INTERVAL 1 HOUR) ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS running_count_remove,
    ...
    FROM
        ...
        GROUP BY 
            interval_start)
    ORDER BY
        decision_timestamp", event_type)
}

pub async fn handle_socket(mut socket: WebSocket, client: Client) {
    debug!("handling ws connection...");
    while let Some(Ok(Message::Text(event_type))) = socket.next().await {
        debug!("got invite for '{}' event type data", event_type);

        let query = mk_query(&event_type);
